...
interface GsiApi {
  accounts: {
    id: {
      initialize: (config: { client_id: string; callback: GsiCallback; auto_select?: boolean }) => void
...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if ((window as unknown as { google?: GsiApi }).google) {
      init()
      return
    }
    ...
    <div className="min-h-[calc(100vh-6rem)] flex items-center justify-center px-5">
      <div className="w-full max-w-sm animate-slide-up text-center">
        ...
        <div className="relative mt-6">
          <div
            ref={googleBtnRef}
            className="absolute inset-0 z-10 opacity-0 [&>*]:w-full [&>*]:h-full"
            style={{ pointerEvents: loading ? 'none' : 'auto' }}
          />
          <div
            aria-hidden="true"
            className="w-full bg-bg-card border border-border py-3 rounded-2xl font-semibold flex items-center justify-center gap-3 text-[14px] press"
          >
