...
function SignInModal({ onClose }: { onClose: () => void }) {
  ...
  async function handleNativeGoogleLogin() {
    ...
    try {
      ...
      if (!idToken) { setError('Google sign-in failed'); setLoading(false); return }
      const res = await fetch('/api/auth/google', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      const data = await res.json()
      if (!res.ok) { setError(data.error || 'Could not authenticate'); setLoading(false); return }
      completeAuth(data.redirectTo)
    } catch (err: unknown) {
  ...
  function googleLogin() {
    if (isNative) handleNativeGoogleLogin()
    else webGoogleLogin()
  }
  ...
  async function handleAppleLogin() {
    ...
    try {
      ...
      if (code === 'SIGN_IN_CANCELED') { setLoading(false); return }
  ...
  return (
    ...
    >
      <div className="w-full max-w-sm bg-bg-card rounded-2xl border border-border shadow-2xl overflow-hidden p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-[18px] font-bold tracking-tight">
            <span className="text-accent">[</span> Rabona <span className="text-accent">]</span>
          </h2>
          <button onClick={onClose} aria-label="Close" className="press text-text-muted hover:text-text p-1">
            <X size={18} />
          </button>
        </div>

        <p className="text-[13px] text-text-muted">Sign in to post, like, and connect with friends.</p>

        {error && (
          <div className="bg-bg-input border border-border text-text rounded-xl px-4 py-2.5 text-[13px] mt-4">
            {error}
          </div>
        )}
        ...
        <button
          onClick={() => googleLogin()}
          disabled={loading}
          className="w-full bg-bg-card border border-border py-3 rounded-2xl font-semibold press flex items-center justify-center gap-3 text-[14px] hover:bg-bg-card-hover transition-colors disabled:opacity-50 mt-4"
        >
        ...
        >
          {showEmail ? 'Hide' : 'Sign in with email & password'}
        </button>
